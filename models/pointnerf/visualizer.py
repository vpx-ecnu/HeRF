...
from models.pointnerf.mvs import *
...
def save_points(xyz, dir, total_steps):
    ...
    for i in range(xyz.shape[0]):
        if isinstance(total_steps,str):
            filename = 'step-{}-{}.ply'.format(total_steps, i)
        else:
            filename = 'step-{:04d}-{}.ply'.format(total_steps, i)
        filepath = os.path.join(dir, filename)
        # np.savetxt(filepath, xyz[i, ...].reshape(-1, xyz.shape[-1]), delimiter=";")
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(xyz[i, ..., :3])
        pcd.colors = o3d.utility.Vector3dVector(xyz[i, ..., -3:])
...
class Visualizer:
    def __init__(self, opt):
        self.opt = opt
        self.log_dir = os.path.join(opt.checkpoints_dir, opt.expname)
        self.image_dir = os.path.join(opt.checkpoints_dir, opt.expname, 'images')
        self.point_dir = os.path.join(opt.checkpoints_dir, opt.expname, 'points')
        self.vid_dir = os.path.join(opt.checkpoints_dir, opt.expname, 'vids')
        ...
        if opt.show_tensorboard > 0:
            ...
            self.tb_writer = SummaryWriter(
                os.path.join(
                    opt.checkpoints_dir, opt.expname,
