...
mse2psnr2 = lambda x: -10.0 * np.log(x) / np.log(10.0)
...
def get_rays_mvs(
    ...
    if isRandom:
        if is_precrop_iters and torch.rand((1,)) > 0.3:
            xs, ys = torch.randint(W // 6, W - W // 6, (N,)).float().to(
                device
            ), torch.randint(H // 6, H - H // 6, (N,)).float().to(device)
        else:
            xs, ys = torch.randint(0, W, (N,)).float().to(device), torch.randint(
                0, H, (N,)
            ).float().to(device)
    else:
        ...
        if chunk > 0:
            ys, xs = (
                ys[idx * chunk : (idx + 1) * chunk],
                xs[idx * chunk : (idx + 1) * chunk],
            )
...
def get_ndc_coordinate(
    ...
    if intrinsic_ref is not None:
        ...
        point_samples_pixel[:, :2] = (
            point_samples_pixel[:, :2] / point_samples_pixel[:, -1:] + 0.0
        ) / inv_scale.reshape(
            1, 2
        )  # normalize to 0~1
        if not lindisp:
            point_samples_pixel[:, 2] = (point_samples_pixel[:, 2] - near) / (
                far - near
            )  # normalize to 0~1
        else:
            point_samples_pixel[:, 2] = (
                1.0 / point_samples_pixel[:, 2] - 1.0 / near
            ) / (1.0 / far - 1.0 / near)
    else:
        # using bounding box
    ...
    if pad > 0:
        ...
        point_samples_pixel[:, 1] = point_samples_pixel[:, 1] * H_feat / (
            H_feat + pad * 2
        ) + pad / (H_feat + pad * 2)
        point_samples_pixel[:, 0] = point_samples_pixel[:, 0] * W_feat / (
            W_feat + pad * 2
        ) + pad / (W_feat + pad * 2)
...
def build_color_volume(
    point_samples, pose_ref, imgs, img_feat=None, downscale=1.0, with_mask=False
):
    """
    ...
    colors = torch.empty(
        (*point_samples.shape[:2], V * C), device=imgs.device, dtype=torch.float
    )
    for i, idx in enumerate(range(V)):
        w2c_ref, intrinsic_ref = (
            pose_ref["w2cs"][idx],
            pose_ref["intrinsics"][idx].clone(),
        )  # assume camera 0 is reference
        point_samples_pixel = get_ndc_coordinate(
            w2c_ref, intrinsic_ref, point_samples, inv_scale
        )[None]
...
def index_point_feature(volume_feature, ray_coordinate_ref, chunk=-1):
    ...
    if chunk != -1:
        features = torch.zeros(
            (volume_feature.shape[1], H, W),
            device=volume_feature.device,
            dtype=torch.float,
            requires_grad=volume_feature.requires_grad,
        )
        grid = (
            ray_coordinate_ref.view(1, 1, 1, H * W, 3) * 2 - 1.0
        )  # [1 1 H W 3] (x,y,z)
        for i in range(0, H * W, chunk):
            features[:, i : i + chunk] = F.grid_sample(
                volume_feature,
                grid[:, :, :, i : i + chunk],
                align_corners=True,
                mode="bilinear",
        ...
        grid = (
            ray_coordinate_ref.view(-1, 1, H, W, 3).to(device) * 2 - 1.0
        )  # [1 1 H W 3] (x,y,z)
        features = (
            F.grid_sample(volume_feature, grid, align_corners=True, mode="bilinear")[
                :, :, 0
            ]
            .permute(2, 3, 0, 1)
            .squeeze()
...
def sub_selete_data(
    ...
    for item in data_batch.keys():
        data_sub_selete[item] = (
            data_batch[item][:, idx].float()
            if (item not in filtIndex and torch.is_tensor(item) and item.dim() > 2)
            else data_batch[item].float()
        )
...
def gen_render_path(c2ws, N_views=30):
    ...
    weight = np.linspace(1.0, 0.0, N_views // 3, endpoint=False).reshape(-1, 1)
    for i in range(N):
        ...
        if i:
            ...
            positions_inteplat.append(
                weight * positions[i - 1] + (1.0 - weight) * positions[i]
            )
    ...
    angles_inteplat, positions_inteplat = np.concatenate(
        rotvec_inteplat
    ), np.concatenate(positions_inteplat)
...
def homo_warp_nongrid(c2w, w2c, intrinsic, ref_cam_xyz, HD, WD, filter=True, **kwargs):
    ...
    if w2c is not None:
        src_cam_xyz = (
            torch.cat([ref_cam_xyz, torch.ones_like(ref_cam_xyz[:, :, 0:1])], dim=-1)
            @ c2w.transpose(1, 2)
            @ w2c.transpose(1, 2)
        )
    else:
    ...
    src_grid = (
        (src_cam_xyz[..., :3] / src_cam_xyz[..., 2:3]) @ intrinsic.transpose(1, 2)
    )[..., :2]
    ...
    mask = (
        torch.prod(
            torch.cat(
                [
                    torch.ge(src_grid, torch.zeros([1, 1, 2], device=src_grid.device)),
                    torch.le(
                        src_grid,
                        torch.tensor([[[WD - 1, HD - 1]]], device=src_grid.device),
                    ),
                ],
                dim=-1,
            ),
    ...
    src_grid = (
        torch.masked_select(src_grid, mask).reshape(B, -1, 2) if filter else src_grid
    )
...
def homo_warp_nongrid_occ(
    c2w, w2c, intrinsic, ref_cam_xyz, HD, WD, tolerate=0.1, scatter_cpu=True
):
    # src_grid: B, 3, D*H*W   xyz
    B, M, _ = ref_cam_xyz.shape
    if w2c is not None:
    ...
    min_depth, argmin = scatter_min(
        cam_z[:, :].cpu() if scatter_cpu else cam_z[:, :],
        index[:, :].cpu() if scatter_cpu else index[:, :],
        dim=1,
    )
    # print("argmin", min_depth.shape, min_depth, argmin.shape)

    queried_depth = (
        min_depth.to(ref_cam_xyz.device)[:, index[0, ...]]
        if scatter_cpu
        else min_depth[:, index[0, ...]]
    )
    ...
    src_grid_x = torch.masked_select(src_grid_x, block_mask[..., None]).reshape(
        B, -1, 1
    )
    src_grid_y = torch.masked_select(src_grid_y, block_mask[..., None]).reshape(
        B, -1, 1
    )
...
def id2mask(hard_id_xy, HD, WD):
    ...
    mask[
        hard_id_xy[0, ..., 1], hard_id_xy[0, ..., 0]
    ] = 1  # torch.ones_like(hard_id_xy[0,...,0], dtype=mask.dtype)
    return mask


def gen_bg_points(batch):
    plane_pnt, plane_normal = batch["plane_pnt"][0], batch["plane_normal"][0]
    plane_pnt, plane_normal = torch.as_tensor(
        plane_pnt, dtype=torch.float32, device=batch["campos"].device
    ), torch.as_tensor(plane_normal, dtype=torch.float32, device=batch["campos"].device)
    cross_xyz_world = get_rayplane_cross(
        batch["campos"],
        batch["raydir"],
        plane_pnt[None, None, :],
...
def extract_from_2d_grid(src_feat, src_grid, mask):
    ...
    warped_src_feat = (
        warped_src_feat.permute(0, 2, 3, 1).view(B, M, src_feat.shape[1]).cuda()
    )  # 1, 224874, 3
    if mask is not None:
        B, N, _ = mask.shape
        full_src_feat = torch.zeros(
            [B, N, src_feat.shape[1]],
            device=warped_src_feat.device,
            dtype=warped_src_feat.dtype,
        )
...
def homo_warp(src_feat, proj_mat, depth_values, src_grid=None, pad=0):
    ...
    warped_src_feat = F.grid_sample(
        src_feat,
        src_grid.view(B, D, H_pad * W_pad, 2),
        mode="bilinear",
        padding_mode="zeros",
        align_corners=True,
    )  # (B, C, D, H*W)
...
def alpha_masking(points, alphas, intrinsics, c2ws, w2cs, near_far, opt=None):
    ...
    for i in range(len(alphas)):
        alpha, intrinsic, c2w, w2c = (
            torch.as_tensor(alphas[i][0], dtype=points.dtype, device=points.device),
            torch.as_tensor(intrinsics, dtype=points.dtype, device=points.device),
            torch.as_tensor(c2ws[i], dtype=points.dtype, device=points.device),
            torch.as_tensor(w2cs[i], dtype=points.dtype, device=points.device),
        )
        # print("w_xyz1",w_xyz1.shape, w2c.shape, intrinsic.shape, alpha.shape)
        cam_xyz = w_xyz1 @ w2c.t()
        torch.cuda.empty_cache()
        if near_far is not None:
            near_far_mask = torch.logical_and(
                cam_xyz[..., 2] >= (near_far[0] - 1.0), cam_xyz[..., 2] <= near_far[1]
            )
        torch.cuda.empty_cache()
        ...
        if opt is not None and (opt.alpha_range > 0 or opt.inall_img == 0):
            range_mask = torch.logical_and(
                img_xy >= torch.zeros((1, 2), dtype=img_xy.dtype, device=img_xy.device),
                img_xy
                < torch.as_tensor([[W, H]], dtype=img_xy.dtype, device=img_xy.device),
            )
        ...
        if near_far is not None:
            vishull_mask = (
                (mask * near_far_mask)
                if vishull_mask is None
                else vishull_mask * (mask * near_far_mask)
            )
        else:
