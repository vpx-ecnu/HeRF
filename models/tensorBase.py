...
def raw2alpha(sigma, dist):
    ...
    alpha = torch.where(torch.isinf(alpha), torch.zeros_like(alpha), alpha)
    # alpha = alpha + 1e-5
    T = torch.cumprod(
        torch.cat(
...
class TensorBase(torch.nn.Module):
    ...
    def update_stepSize(self, gridSize):
        # todo upsample
        gridSize = gridSize * self.n_block_per_axis
        print("aabb", self.aabb.view(-1))
        print("grid size", gridSize)
        self.aabbSize = self.aabb[1] - self.aabb[0]
        self.invaabbSize = 2.0 / self.aabbSize
        self.gridSize = torch.LongTensor(gridSize).to(self.device)
        # self.units = self.block.voxel_size.mean(dim=0)[0]

        self.units = self.aabbSize / (self.gridSize - 1)
        self.stepSize = torch.max(self.units) * self.step_ratio
        self.aabbDiag = torch.sqrt(torch.sum(torch.square(self.aabbSize)))
        self.nSamples = int((self.aabbDiag / self.stepSize).item()) + 1

        # self.units=self.aabbSize / (self.gridSize-1)
        # self.nSamples = int(torch.norm(self.gridSize.float()) / self.step_ratio) + 1
        # self.aabbDiag = torch.sqrt(torch.sum(torch.square(self.aabbSize)))
        # self.stepSize = self.aabbDiag / (self.nSamples-1) * 1.73

        print("sampling step size: ", self.stepSize)
        print("sampling number: ", self.nSamples)

    def assign_block(self, block_list, tensors, device, res_target=-1):
        block = block_list[0].deepcopy()
        blocks = block.cat(block_list[1:])
        self.block = blocks.to(device)
        if res_target != -1:
            self.block.upsample(res_target)
        return

        for param in tensors.keys():
            # tensor = tensors[param]
            # tensor_list = []
            # for i in range(3):
            # t0 = tensor[i][:, :, :1]
            # t = tensor[i][:, :, blocks.reindex_batch_1d_global-1]
            # t = torch.cat([t0, t], dim=2)
            # tensor_list.append(t)
            # tensor_list = torch.nn.ParameterList(tensor_list).to(device)  # todo, will this clip gradiant?
            self.__dict__[param] = tensors[param].to(device)
    ...
    def sample_ray(self, rays_o, rays_d, is_train=True, N_samples=-1):
        # N_samples = N_samples if N_samples>0 else self.nSamples
        N_samples = max(N_samples, self.nSamples)
        ...
        mask_valid = torch.logical_and(~mask_outbbox, interpx > 0)

        return rays_pts, interpx, mask_valid
    ...
    def forward(
        self, rays_chunk, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1, return_raw=False
    ):
        # sample points
        ...
        if ray_valid.any():
            ...
            # torch.save(xyz_sampled[ray_valid], "/media/data/yxy/ed-nerf/logs/tensoir/log_original_blender/debug/xyz_sampled.pt")
            # torch.save(sigma_feature, "/media/data/yxy/ed-nerf/logs/tensoir/log_original_blender/debug/sigma_feature.pt")
            # xyz_sampled[ray_valid][sigma_feature > 10.]
            feature_valid = sigma_feature != 0.0
            validsigma = (
                self.feature2density(sigma_feature[feature_valid]) + 1e-5
            )  # todo debug code
            validindex = torch.nonzero(ray_valid.view(-1))[feature_valid]
            sigma.view(-1)[validindex.view(-1)] = validsigma
        ...
        if app_mask.any():
            ...
            # torch.save(xyz_sampled[app_mask],
            #            "/media/data/yxy/ed-nerf/logs/tensoir/log_original_blender/debug/xyz_sampled_app.pt")
            # torch.save(app_features,
            #            "/media/data/yxy/ed-nerf/logs/tensoir/log_original_blender/debug/app_features.pt")

        if return_raw:
            return sigma, rgb, z_vals, self.distance_scale
