...
def get_ray_dir_scannet(H, W, intrinsic, margin):
    # Get ray directions for all pixels in camera coordinate.
    # not the real ray_d
    intrinsic = intrinsic.numpy()
    px, py = np.meshgrid(
        np.arange(margin, W - margin).astype(np.float32),
        np.arange(margin, H - margin).astype(np.float32))
    x = (px + 0.5 - intrinsic[0, 2]) / intrinsic[0, 0]
    y = (py + 0.5 - intrinsic[1, 2]) / intrinsic[1, 1]
    z = np.ones_like(x)
